...
class Room:
	def __init__(self, alias):
		...
		r = requests.get("https://"+self.visible_domain+"/.well-known/matrix/server")
		if r.status_code == 200:
			self.connection_string = r.json()['m.server']
		else:
			self.connection_string = self.visible_domain+':443'
		r = requests.get("https://"+self.connection_string+"/_matrix/client/r0/directory/room/"+self.alias_urlenc)
		if r.status_code == 200:
	...
	def promote_admin(self, user_id_to_promote, server_admin_access_token):
		...
		r = requests.post("https://"+self.connection_string+"/_synapse/admin/v1/rooms/"+self.alias_urlenc+"/make_room_admin", json=payload, headers=headers)
		if r.status_code == 200:
			print("Promoted "+user_id_to_promote+" admin in room "+self.alias)
			logging.info("Promoted "+user_id_to_promote+" admin in room "+self.alias)
		else:
			print("Failed to promote "+user_id_to_promote+" admin in room "+self.alias, file=sys.stderr)
