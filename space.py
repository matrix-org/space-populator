...
class Space:
	...
	def add_room(self, room):
		...
		r = requests.put("https://"+self.connection_string+"/_matrix/client/r0/rooms/"+self.id_urlenc+"/state/m.space.child/"+room.id_urlenc, json=payload, headers=headers)
		if r.status_code == 200:
			print("Added "+room.alias+" to space "+self.id)
			logging.info("Added "+room.alias+" to space "+self.id)
		else:
			print("Failed to add "+room.alias+" to space "+self.id, file=sys.stderr)
